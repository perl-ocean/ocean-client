...
test "EventEmitter", ->
    expect(4)
    r = new Ocean('//1.2.3.4/wrongurl', null,
                   {protocols_whitelist: []})
    ...
    # Adding the same eventlistener should be indempotent (ocean-client #4).
    single = -> ok(true)
...
test "NoConstructor", ->
    expect(2)
    r = new Ocean('//1.2.3.4/blah', null, {protocols_whitelist: []})
    ok(r instanceof Ocean)
    r.close()
    r = Ocean('//1.2.3.4/blah', null, {protocols_whitelist: []})
    ok(r instanceof Ocean)
    r.close()

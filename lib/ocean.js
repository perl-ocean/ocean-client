...
var Ocean = function(url, dep_protocols_whitelist, options) {
    if (!(this instanceof Ocean)) {
        // makes `new` optional
        return new Ocean(url, dep_protocols_whitelist, options);
    }
...
Ocean.prototype._didMessage = function(data) {
    ...
    that._dispatchMessage(JSON.parse(data));
...
Ocean.prototype._try_next_protocol = function(close_event) {
    ...
    while(1) {
        ...
        if (!Ocean[protocol] ||
            ...
            // var roundTrips = Ocean[protocol].roundTrips || 1;
            ...
            that._transport = new Ocean[protocol](that, trans_url,
                                                   that._base_url);
            return true;
        }
    }
